...
3. **STYLE**: Make the app look amazing. Use Tailwind CSS for styling. Use extensive animations.
4. **PREVIEW**: The user has a live preview.
`;
...
2. **Entry Point**: Ensure the entry point is 'src/index.tsx' which renders the 'App' component to 'document.getElementById("root")'.
...
6. **Exports**: Always use \`export default\` for components to ensure compatibility with the preview system.
7. **Dependencies**: You can import 'react', 'react-dom', 'lucide-react', and 'framer-motion' (for animations) as they are pre-configured.
8. **INITIALIZATION MANDATE**: If the project is empty (no files), you **MUST** immediately generate the following configuration files alongside the source code:
   - \`package.json\`: Use type="module". Scripts: "dev": "vite", "build": "tsc && vite build". Deps: react, react-dom, lucide-react, framer-motion, clsx, tailwind-merge. DevDeps: vite, @vitejs/plugin-react, tailwindcss, postcss, autoprefixer.
   - \`vite.config.ts\`: Standard React+Vite config.
   - \`tsconfig.json\`: Standard Vite+React TS config.
   - \`tailwind.config.js\`: Configured for "./src/**/*.{js,ts,jsx,tsx}".
   - \`postcss.config.js\`: With tailwindcss and autoprefixer.
   - \`index.html\`: Linking to \`/src/index.tsx\`.
...
9. **INITIALIZATION MANDATE**: If the project is empty (no files), you **MUST** immediately generate the following configuration files alongside the source code:
   - \`package.json\`: Scripts: "start": "ng serve", "build": "ng build". Deps: @angular/core, @angular/common, @angular/router, @angular/platform-browser, rxjs, zone.js. DevDeps: @angular/cli, tailwindcss, postcss, autoprefixer.
   - \`angular.json\`: Minimal configuration for build/serve.
   - \`tsconfig.json\`: Standard Angular TS config.
   - \`tailwind.config.js\`: Configured for "./src/**/*.{html,ts}".
   - \`src/index.html\`: Base HTML file.
   - \`src/styles.css\`: With \`@tailwind base; @tailwind components; @tailwind utilities;\`.
`;
  }
};
